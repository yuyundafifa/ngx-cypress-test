...
describe('Our first suite', () => {
    ...
    it('assert property', () => {
        ...
        cy.contains('nb-card','Common Datepicker').find('input').then( input => {
            ...
            // cy.wrap(input).invoke('prop', 'value').should('contain', 'Jan 5, 2022')
        })
    })
    ...
    it('Web tables', () => {
        ...
        cy.get('thead').find('tr').eq(2).then( tableRow => {
            cy.wrap(tableRow).find('[placeholder="First Name"]').type('Ami')
            cy.wrap(tableRow).find('[placeholder="Last Name"]').type('Moeni')
            cy.wrap(tableRow).find('.nb-checkmark').click()
        })
        cy.get('tbody tr').first().find('td').then( tableColumns => {
            cy.wrap(tableColumns).eq(2).should('contain', 'Ami')
            cy.wrap(tableColumns).eq(3).should('contain', 'Moeni')
        })
